...
NSString *const LYSDatePickerDidClearDateNotifition = @"LYSDatePickerDidClearDateNotifition";
...
- (void)clearDatePicker {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:LYSDatePickerDidClearDateNotifition
                                                        object:nil
                                                      userInfo:@{@"date":@""}];

    if (self.didSelectDatePicker)
    {
        self.didSelectDatePicker(nil);
    }

    if (self.delegate && [self.delegate respondsToSelector:@selector(pickerViewControllerDidClear:didSelectDate:)])
    {
        [self.delegate pickerViewControllerDidClear:(LYSDatePickerController *)self didSelectDate:nil];
    }
}
