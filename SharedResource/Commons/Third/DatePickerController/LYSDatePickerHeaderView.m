...
- (LYSDatePickerItem *)clearItem
{
    if (!_clearItem) {
        _clearItem = [LYSDatePickerItem btnWithTitle:@"清空" target:self action:@selector(clearAction:)];
    }
    return _clearItem;
}
...
- (void)clearAction:(LYSDatePickerItem *)sender {
    [self.headerVC clearDatePicker];
    [self.headerVC hiddenDatePicker];
}
...
- (void)customSubViews {
    ...
    [self addSubview:self.clearItem.itemView];
...
- (void)setShowClearBtn:(BOOL)showClearBtn{
    ...
    _clearItem.itemView.hidden = !showClearBtn;
}
- (void)setItemColor:(UIColor *)itemColor
{
    _itemColor = itemColor;
    if (_itemColor) {
        self.centerItem.textColor = itemColor;
        self.leftItem.textColor = itemColor;
        self.clearItem.textColor = itemColor;
        self.rightItem.textColor = itemColor;
    }
}
...
- (void)setHeaderHeight:(CGFloat)headerHeight
{
    ...
    [self.clearItem updateSize];

    CGRect clearItemFrame = self.clearItem.itemView.frame;
    clearItemFrame.origin.x = self.leftItem.itemView.frame.size.width + 15;
    clearItemFrame.origin.y = (_headerHeight - CGRectGetHeight(clearItemFrame))/2.0;
    self.clearItem.itemView.frame = clearItemFrame;
