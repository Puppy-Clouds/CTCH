...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    CGFloat labelHeight = textView.contentSize.height;    
    //限制行数
    int count = (labelHeight) / textView.font.lineHeight;
    
    if (count < _countLine) {
        [self.messageDelegate textViewChangeFrame:self changeHeight:count * ((int)textView.font.lineHeight) + 20];
    }else{
        [self.messageDelegate textViewChangeFrame:self changeHeight:_countLine *((int)textView.font.lineHeight)];
    }
