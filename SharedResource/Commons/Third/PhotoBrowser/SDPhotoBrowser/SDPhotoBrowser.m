...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    if (error) {
//        label.bounds = CGRectMake(0, 0, 250, 60);
//        label.numberOfLines = 2;
//        label.text = @"保存失败 \n 请检查 设置->隐私->照片 中设置是否正确";
        [[[UIAlertView alloc] initWithTitle:@"保存失败" message:@"请检查 设置->隐私->照片 中设置是否正确" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil] show];
    }   else {
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        ...
        imageView.tag = self.imageCount - i - 1;
...
//    NSInteger currentIndex = currentImageView.tag;(修改查看图片返回位置是更改此处)
    NSInteger currentIndex = currentImageView.tag+1;
    NSLog(@"收起：%ld", (long)currentIndex);
...
- (void)showFirstImage
{
    ...
    NSLog(@"展开：%ld", (long)self.currentImageIndex);
    tempView.frame = rect;
